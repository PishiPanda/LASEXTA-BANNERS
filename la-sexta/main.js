...
document.addEventListener('DOMContentLoaded', () => {
    ...
    function drawText(text) {
        // --- TEXTO PRINCIPAL E-SPORTS (AO VIVO STYLE) ---
        ...
        // Gradiente de texto (Fuego / E-Sports) naranja claro -> naranja intenso -> rojo
        const textHeight = fontSize;
        const textGradient = ctx.createLinearGradient(
            CANVAS_WIDTH / 2, (CANVAS_HEIGHT / 2) - (textHeight / 2),
            CANVAS_WIDTH / 2, (CANVAS_HEIGHT / 2) + (textHeight / 2)
        );
        textGradient.addColorStop(0, globalThemeConfig.gradTop);
        textGradient.addColorStop(0.5, globalThemeConfig.gradMid);
        textGradient.addColorStop(1, globalThemeConfig.gradBot);

        ctx.fillStyle = textGradient;

        // Sombra roja/intensa para destacar sobre el fondo oscuro
        ctx.shadowColor = globalThemeConfig.textShadow;
        ctx.shadowBlur = 40;
        ctx.shadowOffsetX = 0;
        ctx.shadowOffsetY = 10;

        // Efecto pseudo-letter-spacing si el navegador lo permite en canvas
